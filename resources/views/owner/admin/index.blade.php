<x-app-layout>
    ...
    <x-slot name="breadcrumb">
        ...
        <x-link-button name="create" href="{{ route('owner.admin.create') }}" />
    </x-slot>
                    ...
                    <tr>
                        ...
                        <td class="border-t-2 border-b-2 border-gray-200 px-4 py-3 flex items-center">
                            @if ($admin->adminProfile)
                                <img src="{{ !empty($admin->adminProfile->admin_photo_path) ? url('storage/admins/' . $admin->adminProfile->admin_photo_path) : url('upload/kids.jpg') }}"
                                    alt="" class="w-12 h-12 rounded-full mr-2">
                                <span>{{ $admin->adminProfile->full_name }}</span>
                            @endif
                        </td>
                        <td class="border-t-2 border-b-2 border-gray-200 px-4 py-3">
                            @if ($admin->adminProfile)
                        ...
                        <td class="border-t-2 border-b-2 border-gray-200 px-4 py-3">
                            @if ($admin->adminProfile)
                                @if ($admin->adminProfile->status === '利用中')
                                    <x-badge name="active" />
                                @else
                        ...
                        <td class="border-t-2 border-b-2 border-gray-200 px-4 py-3 text-lg text-gray-900">
                            @if ($admin->adminProfile)
                                {{ $admin->adminProfile->last_login_at }}
                            @endif
                        </td>
                        <td class="border-t-2 border-b-2 border-gray-200 px-4 py-3 text-center">
                            <x-link-button name="show"
                                href="{{ route('owner.admin.show', ['admin' => $admin->id]) }}" />
                            <x-link-button name="edit"
                                href="{{ route('owner.admin.edit', ['admin' => $admin->id]) }}" />
                            <form method="POST" action="{{ route('owner.admin.destroy', ['admin' => $admin->id]) }}"
                                class="inline">
                                @csrf
                                @method('delete')
